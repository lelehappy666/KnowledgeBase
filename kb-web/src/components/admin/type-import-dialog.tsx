...
import { importTypeAction } from '@/app/actions/type-import'
...
export function TypeImportDialog() {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (!parsedData.definition?.name) {
                throw new Error("缺少必填字段: definition.name")
            }

            // 3. Server Action
            const res = await importTypeAction(parsedData)
            if (res.error) {
    ...
    const exampleJson = {
        "definition": {
            "name": "体感互动装置",
            "code": "TYPE_SENSOR_001"
        },
        "classification": {
            "interactionMethod": ["体感", "光影"],
            "contentType": ["科普", "游戏化互动"],
            "experienceType": ["参与型", "协作型"]
        },
        "technical": {
            "complexity": "Medium",
            "difficulty": 3,
            "developmentCycle": "1-3个月",
            "techRequirements": ["动作捕捉", "实时渲染"]
        },
        "experience": {
            "interactionDepth": "中度互动",
            "participantCount": "2-5人",
            "duration": "1-3分钟",
            "learningCurve": "简单学习"
        },
        "application": {
            "spaceRequirements": ["中型空间", "大型空间"],
            "scenarios": ["常设展览", "教育活动"],
            "audience": ["儿童", "青少年", "成人"]
        },
        "cost": {
            ...
            "updateFrequency": "低频更新"
        },
        "design": {
            "designGuide": "设计原则...",
            "layoutSuggestions": "空间布局...",
            "safetySpecs": ["电源安全", "机械安全"],
            "accessibility": "无障碍设计..."
        },
        "evaluation": {
            "experienceStandard": "体验性标准...",
            "educationStandard": "教育性标准...",
            "technicalStandard": "技术性标准..."
        }
    }
